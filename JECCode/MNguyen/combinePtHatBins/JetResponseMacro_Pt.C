...
//const Int_t nBin = 12 ;
const char* AlgoNames[] = {"ak3PF","ak4PF","ak5PF","akPu3PF","akPu4PF","akPu5PF"};
const Int_t nBin = 6 ;

double jetPtBin[] = {20, 30,40,50,60,70,80,90,100,110,120,130,140,150,160,170,180,190,200,210,220,230,240,250,260,270,280,290,300,340, 400, 500};
...
//  gROOT->ProcessLine(".x betterColors.C");
 ...
 //   TFile *FileA = TFile::Open(Form("jet_CorrPbp502_mergedpthatbins_2013MC.root"));
    TFile *FileA = TFile::Open(Form("jet_CorrPbp502_Combined2013_2014MC.root"));
//    TFile *FileA = TFile::Open(Form("jet_CorrPbp502_mergedpthatbins_YueShiJECv16.root"));
...
//  TFile* outf = new TFile(outname,"recreate");
    TString plotsdir = "/net/hidsk0001/d00/scratch/maoyx/pPb/CMSSW_5_3_8_HI_patch2/src/MNguyen/JEC/testCorrections";
...
//    makeMultiPanelCanvas(c1,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c1,3,1,0.0,0.0,0.1,0.2,0.05);
...
//  makeMultiPanelCanvas(c1,cCanDiv,2,0.0,0.0,0.2,0.15,0.07);
  ...
  TH2D* iHistoCorrPt[nBin];
  TH2D* iHistoRefPt[nBin];
  TH2D* iHistoJtPt[nBin];
  TH2D* iHistoRawPt[nBin];
  TH1D* h1[nBin][CnXbins];
    
  TF1* Gauss[nBin][CnXbins];
 ...
 //   Int_t i = 2 ;
  ...
  //  for(int i =1; i<nCanvDiv-1; i++)
    ...
    TTree* tt2      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[i+cCanDiv]));
     ...
     iHistoCorrPt[i] = new TH2D(Form("%s_CorrPt",AlgoNames[i]),Form("%s_CorrPt",AlgoNames[i]),nJetPtBin,jetPtBin,500,0,5);
      iHistoCorrPt[i+cCanDiv] = new TH2D(Form("%s_CorrPt",AlgoNames[i+cCanDiv]),Form("%s_CorrPt",AlgoNames[i+cCanDiv]),nJetPtBin,jetPtBin,500,0,5);
        iHistoCorrPt[i]->Sumw2();
      iHistoCorrPt[i+cCanDiv]->Sumw2();
  //      t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),"(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>22.)","goff");
        t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),"weight*(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>22.)","goff");
    //    t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),"(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>22.)","goff");
      tt2->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i+cCanDiv]),"weight*(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>22.)","same");
   //   tt2->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i+cCanDiv]),"(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>22.)","same");
 //    t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),Form("corrpt>0 && fabs(jteta)<3.0"),"goff");
      iHistoCorrPt[i]->Draw("colz");
        iHistoCorrPt[i+cCanDiv]->Draw("same colz");
        leg[i]->AddEntry(iHistoCorrPt[i],Form("%s |#eta|<3.0",AlgoNames[i]),"");
      leg[i]->AddEntry(iHistoCorrPt[i+cCanDiv],Form("%s |#eta|<3.0",AlgoNames[i+cCanDiv]),"");
      ...
      format1Dhisto(*iHistoCorrPt[i],250,-1,2,20,2,1,"Jet p_{T}^{ref} (GeV/c)","Corrected p_{T}/ ref p_{T}");//Red      
        format1Dhisto(*iHistoCorrPt[i+cCanDiv],250,-1,2,20,2,1,"Jet p_{T}^{ref} (GeV/c)","Corrected p_{T}/ ref p_{T}");//Red
      leg[i]->Draw();
  //    for(int iX=iHistoCorrPt[i]->GetXaxis()->GetFirst(); iX<iHistoCorrPt[i]->GetXaxis()->GetLast(); iX++ )
          for(int iX=0; iX<nJetPtBin; iX++ )
	{
	  ...
	  h1[i][iX] =(TH1D*)iHistoCorrPt[i]->ProjectionY(Form("h1_%d_%d_py",i,iX),iX,iX,"e");
        h1[i+cCanDiv][iX] = new TH1D(Form("h1_%d_%d_py",i+cCanDiv,iX),Form("h1_%d_%d_py",i+cCanDiv,iX),500,0,5);
        h1[i+cCanDiv][iX] =(TH1D*)iHistoCorrPt[i+cCanDiv]->ProjectionY(Form("h1_%d_%d_py",i+cCanDiv,iX),iX,iX,"e");
...
//    h1[i][iX] =(TH1D*)iHistoCorrPt[i]->ProjectionY(Form("h1_%s_Pt%f_%f_py",AlgoNames[i],jetPtBin[iX],jetPtBin[iX+1]),iHistoCorrPt[i]->GetXaxis()->FindBin(jetPtBin[iX]),iHistoCorrPt[i]->GetXaxis()->FindBin(jetPtBin[iX+1])-1,"e");
	  ...
	  Gauss[i][iX] = new TF1(Form("F6_c%d_d%d",i,iX),"gaus",0,3);
	  Gauss[i][iX]->SetParLimits(1,0.7,1.5);
	  h1[i][iX]->Fit(Form("F6_c%d_d%d",i,iX),"R0WL", "", 0.7, 1.5);
        Gauss[i+cCanDiv][iX] = new TF1(Form("F6_c%d_d%d",i+cCanDiv,iX),"gaus",0,3);
        Gauss[i+cCanDiv][iX]->SetParLimits(1,0.7,1.5);
        h1[i+cCanDiv][iX]->Fit(Form("F6_c%d_d%d",i+cCanDiv,iX),"R0WL", "NO", 0.7, 1.5);
     //   h1[i][iX]->Fit(Gauss[i][iX],"0Q");
        if(DoGausFit){
	  mean[i][iX] = Gauss[i][iX]->GetParameter(1);
	  sigma[i][iX] = Gauss[i][iX]->GetParameter(2);
	  meanErr[i][iX] = Gauss[i][iX]->GetParError(1);
	  sigmaErr[i][iX] = Gauss[i][iX]->GetParError(2);
            mean[i+cCanDiv][iX] = Gauss[i+cCanDiv][iX]->GetParameter(1);
            sigma[i+cCanDiv][iX] = Gauss[i+cCanDiv][iX]->GetParameter(2);
            meanErr[i+cCanDiv][iX] = Gauss[i+cCanDiv][iX]->GetParError(1);
            sigmaErr[i+cCanDiv][iX] = Gauss[i+cCanDiv][iX]->GetParError(2);
        }
        else {
            ...
            mean[i+cCanDiv][iX] = h1[i+cCanDiv][iX]->GetMean();
            sigma[i+cCanDiv][iX]= h1[i+cCanDiv][iX]->GetRMS();
            meanErr[i+cCanDiv][iX] = h1[i+cCanDiv][iX]->GetMeanError();
            sigmaErr[i+cCanDiv][iX] =h1[i+cCanDiv][iX]->GetRMSError();
        }
	  xPoint[i][iX] =iHistoCorrPt[i]->GetXaxis()->GetBinCenter(iX);
	  xPointErr[i][iX] = 0;
        xPoint[i+cCanDiv][iX] =iHistoCorrPt[i+cCanDiv]->GetXaxis()->GetBinCenter(iX);
        xPointErr[i+cCanDiv][iX] = 0;
        ...
        if(i==4 && iX==7)//Just to test 
	    {
	      ...
	      c11->cd();	    
	      h1[i][iX]->Draw();
	      Gauss[i][iX]->Draw("same");
	    }	  
	}

    }    
    if(SavePlot){
        c1->Print(Form("%s/Pbp502JetCorrectedOverRefPt.pdf", plotsdir.Data()));
    }
...
//    makeMultiPanelCanvas(c2,cCanDiv/2,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c2,cCanDiv,1,0.0,0.0,0.1,0.2,0.05);
  ...
  TGraphErrors *CorrPt_Sigma[nBin];
    ...
    makeMultiPanelCanvas(c3,cCanDiv,1,0.0,0.0,0.1,0.2,0.05);
//    makeMultiPanelCanvas(c3,cCanDiv/2,2,0.0,0.0,0.1,0.2,0.05);
  ...
  for(int i2 =0; i2<nCanvDiv; i2++)
    {
      leg2A[i2]= new TLegend(0.32,0.73,0.45,0.9);//top right
      leg2A[i2]->SetFillColor(0);
      leg2A[i2]->SetTextSize(0.05);
      leg2A[i2]->SetBorderSize(0);
      
      leg2B[i2]= new TLegend(0.32,0.73,0.45,0.9);//top right
	...
	{	  
	  ...
	  y3LocErr[iBin] = sigmaErr[i2][iBin];
        PuxLoc[iBin] = xPoint[i2+3][iBin];
        PuxLocErr[iBin] = xPointErr[i2+3][iBin];
        Puy2Loc[iBin] = mean[i2+3][iBin];
        Puy2LocErr[iBin] = meanErr[i2+3][iBin];
        Puy3Loc[iBin] = sigma[i2+3][iBin];
        Puy3LocErr[iBin] = sigmaErr[i2+3][iBin];
	}
...
//     if(i2>=12)c2C->cd(i2-12);
      ...
      CorrPt_Mean[i2] = new TGraphErrors(nXbins,xLoc,y2Loc,xLocErr,y2LocErr);
        CorrPt_Mean[i2+3] = new TGraphErrors(nXbins,PuxLoc,Puy2Loc,PuxLocErr,Puy2LocErr);
      formatTGraph(*CorrPt_Mean[i2],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#mu ");
      formatTGraph(*CorrPt_Mean[i2+3],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#mu ");
        leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form("%s |#eta|<3.0",AlgoNames[i2]),"PL");
      leg2A[i2]->AddEntry(CorrPt_Mean[i2+cCanDiv],Form("%s |#eta|<3.0",AlgoNames[i2+cCanDiv]),"PL");
      leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form("p_{T}^{corrected}/p_{T}^{ref}"),"");
      CorrPt_Mean[i2]->GetXaxis()->SetRangeUser(20., 400.);
      ...
      CorrPt_Mean[i2]->Draw("AP");
        CorrPt_Mean[i2+cCanDiv]->SetMarkerStyle(24);
        CorrPt_Mean[i2+cCanDiv]->SetMarkerColor(2);
        CorrPt_Mean[i2+cCanDiv]->SetLineColor(2);
        CorrPt_Mean[i2+cCanDiv]->SetMarkerSize(1.3);
        CorrPt_Mean[i2+cCanDiv]->Draw("psame");
...
//     if(i2>=12)c3C->cd(i2-12);
      ...
      CorrPt_Sigma[i2+cCanDiv] = new TGraphErrors(nXbins,PuxLoc,Puy3Loc,PuxLocErr,Puy3LocErr);
      formatTGraph(*CorrPt_Sigma[i2],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#sigma ");
        formatTGraph(*CorrPt_Sigma[i2+cCanDiv],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#sigma ");
      leg2B[i2]->AddEntry(CorrPt_Sigma[i2],Form("%s |#eta|<3.0",AlgoNames[i2]),"PL");
        leg2B[i2]->AddEntry(CorrPt_Sigma[i2+cCanDiv],Form("%s |#eta|<3.0",AlgoNames[i2+cCanDiv]),"PL");
      leg2B[i2]->AddEntry(CorrPt_Sigma[i2],Form("p_{T}^{corrected}/p_{T}^{ref}"),"");
      CorrPt_Sigma[i2]->GetXaxis()->SetRangeUser(20., 400.);
      ...
      CorrPt_Sigma[i2]->Draw("AP");
        CorrPt_Sigma[i2+cCanDiv]->SetMarkerStyle(24);
        CorrPt_Sigma[i2+cCanDiv]->SetMarkerColor(2);
        CorrPt_Sigma[i2+cCanDiv]->SetLineColor(2);
        CorrPt_Sigma[i2+cCanDiv]->SetMarkerSize(1.3);
        CorrPt_Sigma[i2+cCanDiv]->Draw("psame");
      leg2B[i2]->Draw();
        regSun(20.,1.,400.,1.,1, 1);

    }
    if(SavePlot){
        c2->Print(Form("%s/Pbp502CorrectedOverRefJetYaxianMean.pdf", plotsdir.Data()));
        c3->Print(Form("%s/Pbp502CorrectedOverRefJetYaxianSigma.pdf",plotsdir.Data())) ;
    }
    c2->Update();
    c3->Update();
}
