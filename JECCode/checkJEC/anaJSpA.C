...
TString model = "HIJING"; //or AMPT generator
const Int_t pthat = 50 ;
...
void hist_class::Write()
{
    ...
    if(doTrackCorrections){
     if(IsMC==kTRUE)   
          out_name=Form("%s%s_IncJetPt%.f_%sCent4BinCorrTrk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%d_trkbin%d_%s",dataType.Data(),model.Data(), leadingjetcut,corrMet.Data(), trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin,intputFile.Data());
       else 
         out_name=Form("%s_IncJetPt%.f_%sCent4BinCorrTrk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%d_%s",dataType.Data(),leadingjetcut,corrMet.Data(), trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin, intputFile.Data());
      }
       else {
    if(IsMC==kTRUE)
            out_name=Form("%s%s_IncJetPt%.f_Trk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%d_%s",dataType.Data(),model.Data(),leadingjetcut,trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin,intputFile.Data());
    else        
            out_name=Form("%s_NoTrigSelIncJetPt%.f_Trk%.fEtaCut%.fLimit%.f_%sbkgJS%sCone%.f_CenBin%d_Nrbin%d_LJbin%dtrkbin%d_%s",dataType.Data(),leadingjetcut,trackcut,etacut*10, etalimit*10,met.Data(), Norm.Data(),conesize*10, nbin,nrbin,nptbin,ntrkptbin, intputFile.Data());       
    }
     
//    TFile *out_file = new TFile(Form("/net/hidsk0001/d00/scratch/maoyx/pAExercise/CMSSW_5_3_7_patch4/src/testRUN/%s",out_name.Data()), "RECREATE");
...
//            intputFile="pPbHijing_5_3_3_v0_HiForest2_v07_WithCentrality.root" ;
         //   intputFile="pPbHijing_5_3_3_v0_HiForest2_v07.root" ;
            ...
            intputFile="HiForest_pythia533.root";
        else 
