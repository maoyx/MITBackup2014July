...
//    TFile *FileA = TFile::Open(Form("/Volumes/WORKDISK/CMS/jet_Corrpp276HITrack_mergedpthatbins_2013MC.root"));
  ...
  //  makeMultiPanelCanvas(c1,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c1,nBin/4,1,0.0,0.0,0.1,0.2,0.05);
...
//  const int cCanDiv = nBin/2;
  const int cCanDiv = nBin/3;
//  makeMultiPanelCanvas(c1,cCanDiv,2,0.0,0.0,0.2,0.15,0.07);
  int nCanvDiv = cCanDiv;
  const int CnXbins = hetab;
    int nXbins = CnXbins;

  TH2D* iHistoCorrEta[nbins];
  TH2D* iHistoCorrEtaPUJet[nbins];
  TH2D* iHistoRefEta[nbins];
  TH2D* iHistoJtPt[nbins];
  TH2D* iHistoRawPt[nbins];
  TH1D* h1[nBin][CnXbins];
  TH1D* hpu[nBin][CnXbins];
    
  TF1* Gauss6[nbins][CnXbins];
  TF1* Gauss6pu[nbins][CnXbins];
  double mean[nbins][CnXbins];
  double pumean[nbins][CnXbins];
  double sigma[nbins][CnXbins];
  double pusigma[nbins][CnXbins];
  double meanErr[nbins][CnXbins];
  double pumeanErr[nbins][CnXbins];
  double sigmaErr[nbins][CnXbins];
  double pusigmaErr[nbins][CnXbins];
  double xPoint[nbins][CnXbins];
  double puxPoint[nbins][CnXbins];
  double xPointErr[nbins][CnXbins];
  double puxPointErr[nbins][CnXbins];
  TLegend*  leg[cCanDiv];

    for(Int_t nj= 0 ; nj <nbins ; nj++){
        for(Int_t ipt = 0 ; ipt < nXbins ; ipt++){
            ...
            pusigmaErr[nj][ipt] =0.;
            puxPoint[nj][ipt] = 0.;
            puxPointErr[nj][ipt] =0.;
        }
    }
 //   Int_t i = 2 ;
  
    TTree* t      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[0]));
        TTree* tt2      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[6]));
    
    for(int i =0; i<nbins; i++){

        iHistoCorrEta[i] = new TH2D(Form("%s_CorrEtaPtBin%.f_%.f",AlgoNames[0], ptbins[i],ptbins[i+1]),Form("%s_CorrEtaPtBin%.f_%.f",AlgoNames[i], ptbins[i],ptbins[i+1]),nXbins,etabins,500,0,5);
        iHistoCorrEtaPUJet[i] = new TH2D(Form("%s_CorrEtaPUJetPtBin%.f_%.f",AlgoNames[6],ptbins[i],ptbins[i+1]),Form("%s_CorrEtaPUJetPtBin%.f_%.f",AlgoNames[6],ptbins[i],ptbins[i+1]),nXbins,etabins,500,0,5);
       t->Draw(Form("corrpt/refpt:refeta>>%s_CorrEtaPtBin%.f_%.f",AlgoNames[0],ptbins[i],ptbins[i+1]),Form("weight*(corrpt>0 && refpt>=%.f && refpt<%.f && fabs(jteta)<3.0 && rawpt>22.)", ptbins[i],ptbins[i+1]),"goff");
        tt2->Draw(Form("corrpt/refpt:refeta>>%s_CorrEtaPUJetPtBin%.f_%.f",AlgoNames[6], ptbins[i],ptbins[i+1]),Form("weight*(corrpt>0 && refpt>=%.f && refpt<%.f  && fabs(jteta)<3.0 && rawpt>22.)", ptbins[i],ptbins[i+1]),"same");
        format1Dhisto(*iHistoCorrEta[i],250,-1,2,20,2,1,"Jet #eta","Corrected p_{T}/ ref p_{T}");//Red      
        format1Dhisto(*iHistoCorrEtaPUJet[i],250,-1,2,20,2,1,"Jet #eta","Corrected p_{T}/ ref p_{T}");//Red
       for(int iX=iHistoCorrEta[i]->GetXaxis()->GetFirst(); iX<iHistoCorrEta[i]->GetXaxis()->GetLast(); iX++ )
        {
            ...
            hpu[i][iX] = new TH1D(Form("hpu_%d_%d_py",i,iX),Form("hpu_%d_%d_py",i,iX),500,0,5);
            hpu[i][iX] =(TH1D*)iHistoCorrEtaPUJet[i]->ProjectionY(Form("hpu_%d_%d_py",i,iX),iX,iX,"e");
            Gauss6pu[i][iX] = new TF1(Form("F6pu_c%d_d%d",i,iX),"gaus",0,5);
            Gauss6pu[i][iX]->SetParLimits(1,0.7,1.5);
            hpu[i][iX]->Fit(Form("F6pu_c%d_d%d",i,iX),"RQ0WL", "NO", 0.7, 1.5);
     //   h1[i][iX]->Fit(Gauss6[i][iX],"0Q");
        if(DoGausFit){
          ...
          sigmaErr[i][iX] = Gauss6[i][iX]->GetParError(2);
            pumean[i][iX] = Gauss6pu[i][iX]->GetParameter(1);
            pusigma[i][iX] = Gauss6pu[i][iX]->GetParameter(2);
            pumeanErr[i][iX] = Gauss6pu[i][iX]->GetParError(1);
            pusigmaErr[i][iX] = Gauss6pu[i][iX]->GetParError(2);
        }
        else {
            ...
            pumean[i][iX] = hpu[i][iX]->GetMean();
            pusigma[i][iX]= hpu[i][iX]->GetRMS();
            pumeanErr[i][iX] = hpu[i][iX]->GetMeanError();
            pusigmaErr[i][iX] =hpu[i][iX]->GetRMSError();
        }
         xPoint[i][iX] =iHistoCorrEta[i]->GetXaxis()->GetBinCenter(iX);
          xPointErr[i][iX] = 0;
            puxPoint[i][iX] =iHistoCorrEtaPUJet[i]->GetXaxis()->GetBinCenter(iX);
            puxPointErr[i][iX] = 0;
 ...
 //       t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("(rawpt>15 && refpt>30 && fabs(jteta)<3.0)"),"goff");
        iHistoCorrEta[i*2+1]->Draw("colz");
        iHistoCorrEtaPuJet[i*2+1]->Draw("same colz");
        leg[i]->AddEntry(iHistoCorrEta[i],Form("%s |#eta|<3.0",AlgoNames[0]),"");
        leg[i]->AddEntry(iHistoCorrEtaPUJet[i],Form("%s |#eta|<3.0",AlgoNames[6]),"");
  ...
  TGraphErrors *CorrPt_Mean[nbins];
  TGraphErrors *CorrPt_Meanpu[nbins];
    TCanvas *c2 = new TCanvas("c2","c2",10,10,kw,kh);
 //   makeMultiPanelCanvas(c2,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c2,nBin/3,1,0.0,0.0,0.1,0.2,0.05);
  ...
  TGraphErrors *CorrPt_Sigma[nbins];
  TGraphErrors *CorrPt_Sigmapu[nbins];
    TCanvas *c3 = new TCanvas("c3","c3",10,10,kw,kh);
  //  makeMultiPanelCanvas(c3,nBin/4,2,0.0,0.0,0.1,0.2,0.05);
    makeMultiPanelCanvas(c3,nBin/3,1,0.0,0.0,0.1,0.2,0.05);
  ...
  for(int i2 =0; i2<nbins; i2++)
	...
	{	  
	  ...
	  y3LocErr[iBin] = sigmaErr[i2][iBin];
        PuxLoc[iBin] = puxPoint[i2][iBin];
        PuxLocErr[iBin] = puxPointErr[i2][iBin];
        Puy2Loc[iBin] = pumean[i2][iBin];
        Puy2LocErr[iBin] = pumeanErr[i2][iBin];
        Puy3Loc[iBin] = pusigma[i2][iBin];
        Puy3LocErr[iBin] = pusigmaErr[i2][iBin];
	}
      CorrPt_Mean[i2] = new TGraphErrors(nXbins,xLoc,y2Loc,xLocErr,y2LocErr);
        CorrPt_Meanpu[i2] = new TGraphErrors(nXbins,PuxLoc,Puy2Loc,PuxLocErr,Puy2LocErr);
      formatTGraph(*CorrPt_Mean[i2],250,-1,1,20,1,1,"Jet #eta","#mu ");
    formatTGraph(*CorrPt_Meanpu[i2],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#mu ");
   }
...
//     if(i2>=12)c2C->cd(i2-12);
      c2->cd(i2+1);
      leg2A[i2]->AddEntry(CorrPt_Mean[i2*2+1],Form("%s |#eta|<3.0",AlgoNames[0]),"PL");
        leg2A[i2]->AddEntry(CorrPt_Meanpu[i2*2+1],Form("%s |#eta|<3.0",AlgoNames[6]),"PL");
    leg2A[i2]->AddEntry(CorrPt_Mean[i2],Form("p_{T}^{ref} > 50 GeV/c"),"");
     ...
     CorrPt_Mean[i2]->GetXaxis()->SetRangeUser(-3., 3.);
      ...
      CorrPt_Mean[i2]->SetMinimum(0.84);
        ...
        CorrPt_Meanpu[i2]->SetMarkerStyle(24);
        CorrPt_Meanpu[i2]->SetMarkerColor(2);
        CorrPt_Meanpu[i2]->SetLineColor(2);
        CorrPt_Meanpu[i2]->SetMarkerSize(1.3);
        CorrPt_Meanpu[i2]->Draw("psame");
      leg2A[i2]->Draw();
        regSun(-4.,1.,4.,1.,1, 1);
...
//     if(i2>=12)c3C->cd(i2-12);
      ...
      CorrPt_Sigma[i2] = new TGraphErrors(nXbins,xLoc,y3Loc,xLocErr,y3LocErr);
        CorrPt_Sigmapu[i2] = new TGraphErrors(nXbins,PuxLoc,Puy3Loc,PuxLocErr,Puy3LocErr);
      formatTGraph(*CorrPt_Sigma[i2],250,-1,1,20,1,1,"Jet #eta","#sigma ");
        formatTGraph(*CorrPt_Sigmapu[i2],250,-1,1,20,1,1,"p_{T}^{ref} (GeV/c)","#sigma ");
      leg2B[i2]->AddEntry(CorrPt_Sigma[i2],Form("%s |#eta|<3.0",AlgoNames[0]),"PL");
    leg2B[i2]->AddEntry(CorrPt_Sigmapu[i2],Form("%s |#eta|<3.0",AlgoNames[6]),"PL");
      ...
      CorrPt_Sigma[i2]->GetXaxis()->SetRangeUser(-3., 3.);
        ...
        CorrPt_Sigmapu[i2]->SetMarkerStyle(24);
        CorrPt_Sigmapu[i2]->SetMarkerColor(2);
        CorrPt_Sigmapu[i2]->SetLineColor(2);
        CorrPt_Sigmapu[i2]->SetMarkerSize(1.3);
        CorrPt_Sigmapu[i2]->Draw("psame");
      leg2B[i2]->Draw();
        regSun(-3.,1.,3.,1.,1, 1);
