...
void weight_PtHatBins_JetsR3_5(const char *infile="input.root", const char *outfile = "output.root",float maxpthat=30., float xSection=1.,bool updateCorrections=true)
{
  ...
  const std::string inputDirName[ndir]= {
    "ak3PFJetAnalyzer",
    "ak4PFJetAnalyzer",
    "ak5PFJetAnalyzer",
    "ak3CaloJetAnalyzer",
  ...
  const std::string outputDirName[ndir]= {
    "ak3PFJetAnalyzer",
    "ak4PFJetAnalyzer",
    "ak5PFJetAnalyzer",
    "ak3CaloJetAnalyzer",
 ...
 const  std::string calgo[ndir]= {
  ...
  const std::string corrFileName[ndir]= {
    "AK3PF",
    "AK4PF",
    "AK5PF",
    "AK3Calo",
...
//  for(Int_t idir=2;idir<ndir;idir++){
  ...
  //    L2Name = "/net/hidsk0001/d00/scratch/maoyx/pPb/CMSSW_5_3_8_HI_patch2/src/MNguyen/JEC/txt/pp502/Pbp/YueShi_v16/Pbp_PythiaZ2_5020GeV_538HIp2_v16_L2Relative_"+corrFileName[idir]+"_offline.txt";
      L2Name = "/net/hidsk0001/d00/scratch/maoyx/pPb/CMSSW_5_3_8_HI_patch2/src/MNguyen/JEC/Pbp_PythiaZ2_5020GeV_538HIp2_v17_L2Relative_"+corrFileName[idir]+".txt";
...
//      L3Name = "/net/hidsk0001/d00/scratch/maoyx/pPb/CMSSW_5_3_8_HI_patch2/src/MNguyen/JEC/txt/pp502/Pbp/YueShi_v16/Pbp_PythiaZ2_5020GeV_538HIp2_v16_L3Absolute_"+corrFileName[idir]+"_offline.txt";
      L3Name = "/net/hidsk0001/d00/scratch/maoyx/pPb/CMSSW_5_3_8_HI_patch2/src/MNguyen/JEC/Pbp_PythiaZ2_5020GeV_538HIp2_v17_L3Absolute_"+corrFileName[idir]+".txt";
   ...
   //    if(idir!=11) continue ; 
	...
	if(updateCorrections){
	  ...
	  _JEC_HI44X->setJetPt(rawpt[iref]);
           double corrValue = _JEC_HI44X->getCorrection() ;
	  corrpt[iref] = (rawpt[iref]*_JEC_HI44X->getCorrection()); 
	//  corrpt[iref] = (rawpt[iref]*corrValue); 
    ...
    //    if(TMath::Abs(corrpt[iref]-(rawpt[iref]*_JEC_HI44X->getCorrection()))>1. && rawpt[iref]<22.) cout<<"*****!!!!!"<<endl;
          if(TMath::Abs(corrValue-(corrpt[iref]/rawpt[iref]))>1. && rawpt[iref]<22. && corrpt[iref]>40) cout<<"iref: "<<iref<<" correction is: "<<(corrValue)<<" ratio of corr/raw: "<<(corrpt[iref]/rawpt[iref])<<" for rawpt: "<<rawpt[iref]<<" corrpt: "<<corrpt[iref]<<" and rawpT*correction: "<<(rawpt[iref]*corrValue)<<" and correction*rawpt: "<<(corrValue*rawpt[iref])<<" and eta: "<<jteta[iref]<<endl;
	}
      }//ref for loop
      tr_out[idir]->Fill();
    }//nentresi loop
