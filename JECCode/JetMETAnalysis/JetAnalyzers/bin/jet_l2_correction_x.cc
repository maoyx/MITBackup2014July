        ...
        if (ipt==hl_jetpt.nobjects(1)-1 && (vabsrsp_eta.back())->GetN()!=0 && (vabscor_eta.back())->GetN()!=0) {
           ...
           if (npoints<3) {
             ...
             if (alg.find("PF")!=string::npos) {
                ...
                fabscor->SetParLimits(0,0.,1.);
                fabscor->SetParLimits(1,-1.,5.);
                fabscor->SetParLimits(2,1.,20.);
             //   fabscor->SetParLimits(2,0.1,100);
             //   fabscor->SetParLimits(3,-100,0);
                fabscor->SetParLimits(3,0.1,1.5);
             //   fabscor->SetParLimits(4,0,100);
                fabscor->SetParLimits(4,-0.1,0.5);
                fabscor->SetParLimits(5,-0.5,1.5);
              ...
              //original Calo jet fitting functions
               if (xmin<6) xmin=6;
               fabscor=new TF1("fit","[0]+[1]/(pow(log10(x),[2])+[3])",xmin,xmax);
               fabscor->SetParameter(0,0.5);
               fabscor->SetParameter(1,0.5);
               fabscor->SetParameter(2,-0.5);
               fabscor->SetParameter(3,-0.1);

             //  fabscor->SetParLimits(3,0,100);
               fabscor->SetParLimits(2,-1.5,3.);
               fabscor->SetParLimits(3,-0.5,0.5);
            /*
              //test using PF fitting functions for Calo jets
                   fabscor=new TF1("fit","[0]+[1]/(pow(log10(x),2)+[2])+[3]*exp(-[4]*(log10(x)-[5])*(log10(x)-[5]))",xmin,xmax);
                fabscor->SetParameter(0,0.5);
                fabscor->SetParameter(1,9.0);
                fabscor->SetParameter(2,8.0);
...
*/
  ...
  //                fabscor->FixParameter(5,0.0);
      ...
      cout<<"The Fit function used is: "<<fabscor->GetExpFormula()<<endl;
           }
...
string fnc_as_str;
  //  if (alg.find("PF")!=string::npos)  fnc_as_str =    "(x>=20)*([0]+[1]/(pow(log10(x),2)+[2])+[3]*exp(-[4]*(log10(x)-[5])*(log10(x)-[5])))+(x<20)*1";
    if (alg.find("PF")!=string::npos)  fnc_as_str =    "([0]+[1]/(pow(log10(x),2)+[2])+[3]*exp(-[4]*(log10(x)-[5])*(log10(x)-[5])))";
 //  else  if (alg.find("Calo")!=string::npos) fnc_as_str =  "(x>=20)*([0]+[1]/(pow(log10(x),[2])+[3]))+(x<20)*1";
   else  if (alg.find("Calo")!=string::npos) fnc_as_str =  "([0]+[1]/(pow(log10(x),[2])+[3]))";
 //  else  if (alg.find("Calo")!=string::npos) fnc_as_str =  "(x>=20)*([0]+[1]/(pow(log10(x),2)+[2])+[3]*exp(-[4]*(log10(x)-[5])*(log10(x)-[5])))+(x<20)*1";
   else  if(alg.find("trk")!=string::npos) fnc_as_str =  "[0]+[1]*log10(x)+[2]*pow(log10(x),2)+[3]*pow(log10(x),3)+[4]*pow(x/500.0,3)" ; 
    else fnc_as_str = "[0]+[1]*log10(x)+[2]*pow(log10(x),2)+[3]*pow(log10(x),3)+[4]*pow(log10(x),4)"; 

    cout<<"for txt printed the TF1: "<<fnc_as_str.c_str()<<endl;
