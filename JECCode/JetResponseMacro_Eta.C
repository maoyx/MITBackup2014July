...
//  makeMultiPanelCanvas(c1,cCanDiv,2,0.0,0.0,0.2,0.15,0.07);
  ...
  //  for(int i =1; i<nCanvDiv-1; i++)
    ...
    TTree* t      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[i]));
      ...
      leg[i]->SetBorderSize(0);
        ...
        iHistoCorrEta[i] = new TH2D(Form("%s_CorrEta",AlgoNames[i]),Form("%s_CorrEta",AlgoNames[i]),nXbins,etabins,500,0,5);
 ...
 //       t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("(rawpt>15 && refpt>30 && fabs(jteta)<3.0)"),"goff");
   //    t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("weight*(corrpt>0 && refpt>50 && fabs(jteta)<3.0 && rawpt>15.)"),"goff");
       t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("(corrpt>0 && refpt>50 && fabs(jteta)<3.0 && rawpt>15.)"),"goff");
    ...
    if(SavePlot){
        c1->Print(Form("%s/pp276CorrectedEtaOverRefJet.pdf", plotsdir.Data()));
    }
...
//     if(i2>=12)c3C->cd(i2-12);
    ...
    if(SavePlot){
        c2->Print(Form("%s/pp276CorrectedEtaOverRefJetMean.pdf", plotsdir.Data()));  
        c3->Print(Form("%s/pp276CorrectedEtaOverRefJetSigma.pdf",plotsdir.Data())) ; 
    }
    c2->Update();
    c3->Update();
}
