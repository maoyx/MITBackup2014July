...
//const int npthat = 8;
//int valpthat[npthat] = {15, 30, 50, 80, 120,170, 220, 280};
const int npthat = 1;
int valpthat[npthat] = {15};
TCanvas *cv1[npthat];
TCanvas *cv2[npthat];
TCanvas *cv3[npthat];
TCanvas *cv4[npthat];
...
TCut genPtMin("refpt > 0");
TCut simTrk(" pPt >0.5 && TMath::Abs(pEta) < 2.4");
TCut matchTrk(" pPt >0.5 && TMath::Abs(pEta) < 2.4 && pNRec >0 ");
HiForest * t[npthat];
HiForest * tref[npthat];
...
// t[ipthat] = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pPb/HP04/prod16/Signal_Pythia_pt%d/HiForest_v77_merged01/pt%d_HIJING_GEN-SIM_revrese_betap_merged_forest_0.root",valpthat[ipthat],valpthat[ipthat]),"PYTHIA",cPPb);
// t[ipthat] = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pPb/HP04/prod16/Signal_Pythia_pt%d/HiForest_v77_v2_merged01/pt%d_HP04_hiforest77_hiSignal.root",valpthat[ipthat],valpthat[ipthat]),"PYTHIA",cPP);
// t[ipthat] = new HiForest(Form("/mnt/hadoop/cms/store/user/dgulhan/pPb/HP04/prod16/Signal_Pythia_pt%d/HiForest_v77_v2_merged02/pt%d_HP04_hiforest77_hiSignal.root",valpthat[ipthat],valpthat[ipthat]),"PYTHIA",cPP);
 t[ipthat] = new HiForest(Form("/mnt/hadoop/cms/store/user/kjung/Pbp_ForestProdApr14_pthat%d_v3/hiForestTree_PbpMC_5_2_Pay.root",valpthat[ipthat]),"HIJINGPYTHIA",cPPb);
 tref[ipthat] = new HiForest(Form("/mnt/hadoop/cms/store/user/kjung/PbpMC/HP06/mergedPthat%d/PbpJEC_pthat%d_outputMerged.root",valpthat[ipthat], valpthat[ipthat]),"HIJINGPYTHIA",cPPb);

 t[ipthat]->InitTree();
 tref[ipthat]->InitTree();
 ...
 tref[ipthat]->Draw("zVtx[maxVtx]","","same ehist");
...
*/
 ...
 cp1[ipthat]->SetLogy();
 t[ipthat]->Draw("pthat","","");
 tref[ipthat]->Draw("pthat","","same ehist");
 l.Draw("same");
 cp1[ipthat]->SaveAs(Form("plots/cp1_%d.png",ipthat));
/*
 // sim track information validation
 ctr1[ipthat] = new TCanvas(Form("ctr1_%d",ipthat),"",600,600);
 ctr1[ipthat]->SetLogy();
 t[ipthat]->Draw("pPt",simTrk,"");
 ctr1[ipthat]->SaveAs(Form("plots/ctr1_%d.png",ipthat));

 ctr2[ipthat] = new TCanvas(Form("ctr2_%d",ipthat),"",600,600);
 ctr2[ipthat]->SetLogy();
 t[ipthat]->Draw("pPt",matchTrk,"");
 ctr2[ipthat]->SaveAs(Form("plots/ctr2_%d.png",ipthat));
*/
}
