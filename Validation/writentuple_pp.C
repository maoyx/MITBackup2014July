...
public:
    ...
    void MakeList(){
        ...
        for(int i = 0;i<t->GetEntries();i++){
            ...
            if(occur>0) cout <<"duplicated found: "<<i<<" / "<<t->GetEntries()<<" run: "<<run<<" evt: "<<evt<<endl;
...
TStopwatch timer;
int writentuple_pp(char *ksp="ppJet80")
{
    ...
    else if(strcmp(ksp,"ppJet80")==0)inname = "/mnt/hadoop/cms/store/user/maoyx/hiForest/PP2013_HiForest_PromptReco_JsonPP_Jet80_HIReco_forestv84_v2.root";
    ...
    //! Output file
    //! HIHighPt
    TFile *fout = new TFile(Form("ntuple_2013ppHIReco_%s.root",ksp),"RECREATE");
  ...
  //  c->hasAkPu3CaloJetTree=0;
   ...
   Long64_t nentries = c->GetEntries();
    ...
    string jetVars = "";
    jetVars += "evt:run:vz:trig:jet40:jet60:jet80:jet100:ntrk:nrefe:pt:raw:eta:phi:chMax:chSum:phSum:neSum";
    TNtuple *ntjet=0;
    ...
    for (Long64_t ievt=0; ievt<nentries;ievt++) {//! event loop
        ...
        if(strcmp(ksp,"ppJet40")==0){
            ...
            evSel = fabs(c->evt.vz)<15. && c->skim.pHBHENoiseFilter  && c->skim.pPAcollisionEventSelectionPA && (c->hlt.HLT_PAJet80_NoJetID_v1 );
            trig=2;
        }
        if(!evSel)continue;
        
        float pt = -9,raw    = -9,eta    = -9,phi    = -9,chMax  = -9,chSum  = -9,phSum  = -9,neSum  = -9,nrefe = 0;
        ...
        */
        for (int i = 0; i<mJets->nref; i++) {
            ...
            float jentry[] = {evt,run,vz,trig,jet40,jet60,jet80,jet100,ntrk,nrefe,
                pt,raw,eta,phi,chMax,chSum,phSum,neSum
            };
