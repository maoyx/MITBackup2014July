...
int JetEnergyResolution(const char *cmode="pythia",int rfit=0)
{
  int statop=0;
  int kRebin=1;
  ...
  //! Input files 
  TFile *fin_pPb     = new TFile("../Output/Response_pPb_test_merged.root","r");
//  TFile *fin_pPb     = new TFile("../Output/Response_pPb_test_pT80_0.root","r");
  ...
  double ptbins[] = {10,15,20,25,30,40,50,60,70,80,90,100,120,140,160,180,200,250,300};
  ...
  for(int nj=0;nj<knj;nj++){
    for(int ic=0;ic<ncen;ic++){
      MakeHistMean(hArM_pp  [nj][ic],1.088,0.94);
      MakeHistRMS (hRMS_pp  [nj][ic],0.43,0.0001);
      MakeHistMean(hMean_pp [nj][ic],1.088,0.94);
      MakeHistRMS (hSigma_pp[nj][ic],0.43,0.0001);

      MakeHistMean(hArM_r_pp  [nj][ic],1.088,0.94);
      MakeHistRMS (hRMS_r_pp  [nj][ic],0.43,0.0001);
      MakeHistMean(hMean_r_pp [nj][ic],1.088,0.94);
...
//  for(int nj=1;nj<7;nj++){
  for(int nj=2;nj<6;nj++){
	...
	TText *text4 = pt3->AddText("PYTHIA 30");
