...
ppMode_(false)
{
   ...
   if (name.Contains("Forest2STA")) {
      ...
      centBin_.push_back("20to28");
      centBin_.push_back("28to40");
   }
...
//             hnameNoE = Form("hPtHatBeforeSel");
   ...
   for (UInt_t s=0; s<ptHatMin_.size(); ++s) { // merge pt hat samples with weight
     if (corrSetName_=="Forest2STAv12") {
        ...
        if ( ptHatMin_[s]==50 ) sampleCroSec_[s] = 1.025E-03 - 9.865E-05;
        else if ( ptHatMin_[s]==80 ) sampleCroSec_[s] = 9.865E-05 - 3.069E-05;
        else if ( ptHatMin_[s]==100) sampleCroSec_[s] = 3.069e-05 - 1.129E-05;
        else if ( ptHatMin_[s]==120) sampleCroSec_[s] = 1.129E-05 - 1.465E-06;
        else if ( ptHatMin_[s]==170) sampleCroSec_[s] = 1.465E-06 - 5.310e-07;
        ...
        if ( ptHatMin_[s] == 30    ) sampleCroSec_[s] = 1.075E-02 - 1.025E-03;
        else if ( ptHatMin_[s]==50 ) sampleCroSec_[s] = 1.025E-03 - 9.865E-05;
        else if ( ptHatMin_[s]==80 ) sampleCroSec_[s] = 9.865E-05 - 3.069E-05;
        else if ( ptHatMin_[s]==100) sampleCroSec_[s] = 3.069e-05 - 1.129E-05;
        else if ( ptHatMin_[s]==120) sampleCroSec_[s] = 1.129E-05 - 1.465E-06;
        else if ( ptHatMin_[s]==170) sampleCroSec_[s] = 1.465E-06 - 5.310e-07;
     ...
     if (ppMode_) {
        ...
        else if ( ptHatMin_[s]==80 ) sampleCroSec_[s] = 9.865E-05 - 1.129E-05;
      //  else if ( ptHatMin_[s]==50 ) sampleCroSec_[s] = 1.021e-03 - 9.913e-05;
        else if ( ptHatMin_[s]==120) sampleCroSec_[s] = 1.129E-05 - 1.465E-06;
        else if ( ptHatMin_[s]==170) sampleCroSec_[s] = 1.465E-06 - 2.837E-07; //upper limit is 220
     //   else if ( ptHatMin_[s]==170) sampleCroSec_[s] = 1.465E-06 - 5.310E-07; //upper limit is 200 
        else if ( ptHatMin_[s]==200) sampleCroSec_[s] = 5.310e-07 - 1.192e-07;
        else if ( ptHatMin_[s]==250) sampleCroSec_[s] = 1.192e-07 - 3.176e-08;
       else if ( ptHatMin_[s]==300) sampleCroSec_[s] = (3.176e-08);
        else if ( ptHatMin_[s]==220) sampleCroSec_[s] = 2.837E-07 - 5.323E-08;
        else if ( ptHatMin_[s]==280) sampleCroSec_[s] = 5.323E-08;
        else cout << endl << endl << " Error no such pt hat!!!!!" << endl << endl << endl;
     }
     float basePtHat;
...
Float_t TrackingCorrections::GetCorr(Float_t pt, Float_t eta, Float_t jet, Float_t cent, Double_t * outCorr)
{
   ...
   if (corrSetName_.Contains("Forest2STAv14")) {
       if (cent<4) bin = 0;
       else if (cent<12) bin = 1;
       else if (cent<20) bin = 2;
       else if (cent<28) bin = 3; 
      else  bin = 4;   
   } else {
      if (cent<2) bin = 0;
   ...
   if (corr[0]<0.001 && jet < 500. && pt < jet) {
     cout << " warning eff=0 for pt eta jet cent: " << pt << " " << eta << " " << jet << " " << cent << endl;
   }
   if(corr[0]<0.001) {
       cout <<"taking the neighbour bin efficiency since eff = 0" <<endl ;
       if(ptBin > 2 && jetBin >2 ) corr[0] = correctionHists_[0][bin]->GetBinContent(etaBin,ptBin-1,jetBin-1);
       else corr[0] = correctionHists_[0][bin]->GetBinContent(etaBin,ptBin, 0);
    }
