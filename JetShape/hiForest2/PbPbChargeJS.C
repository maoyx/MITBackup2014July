...
const int nptbin = 5 ;
const double pt[]={80., 100., 120., 150., 200., 500.};
...
public:
    ...
    TH2F * SyDiffJS[nbin][nrbin]; //differential jet shapes (pho(r)) hist.
    TH2F * AsyDiffJS[nbin][nrbin]; //differential jet shapes (pho(r)) hist.
    TH2F * bkgSyDiffJS[nbin][nrbin]; //differential jet shapes (pho(r)) hist.
    TH2F * bkgAsyDiffJS[nbin][nrbin]; //differential jet shapes (pho(r)) hist.
...
hist_class::hist_class(TString the_desc)
{
    ...
    for(int ibin = 0 ; ibin <nbin; ibin++){
        ...
        for(int ir = 0 ; ir <nrbin; ir++){
            ...
            SyDiffJS[ibin][ir] = new TH2F(Form("symmetryDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), Form("symmetryDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), 500, 0., 500, 150, -0.005, 1.495);
            SyDiffJS[ibin][ir]->GetXaxis()->SetTitle("p_{T}^{jet} (GeV/c)");
            SyDiffJS[ibin][ir]->GetYaxis()->SetTitle("#rho (r)");
            SyDiffJS[ibin][ir]->Sumw2();
            
            AsyDiffJS[ibin][ir] = new TH2F(Form("asymmetricDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), Form("asymmetricDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), 500, 0., 500, 150, -0.005, 1.495);
            AsyDiffJS[ibin][ir]->GetXaxis()->SetTitle("p_{T}^{jet} (GeV/c)");
            AsyDiffJS[ibin][ir]->GetYaxis()->SetTitle("#rho (r)");
            AsyDiffJS[ibin][ir]->Sumw2();

            bkgSyDiffJS[ibin][ir] = new TH2F(Form("symmetricbkgDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), Form("symmetricbkgDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), 500, 0., 500, 150, -0.005, 1.495);
            bkgSyDiffJS[ibin][ir]->GetXaxis()->SetTitle("p_{T}^{jet} (GeV/c)");
            bkgSyDiffJS[ibin][ir]->GetYaxis()->SetTitle("#rho (r)");
            bkgSyDiffJS[ibin][ir]->Sumw2();
            
            bkgAsyDiffJS[ibin][ir] = new TH2F(Form("asymmetrybkgDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), Form("asymmetrybkgDiffJSdR%.f_%.f_Cen%d-%d%%",rbin[ir]*100,rbin[ir+1]*100,centr[ibin],centr[ibin+1]), 500, 0., 500, 150, -0.005, 1.495);
            bkgAsyDiffJS[ibin][ir]->GetXaxis()->SetTitle("p_{T}^{jet} (GeV/c)");
            bkgAsyDiffJS[ibin][ir]->GetYaxis()->SetTitle("#rho (r)");
            bkgAsyDiffJS[ibin][ir]->Sumw2();
...
void hist_class::Delete()
{
    for(int ibin = 0 ; ibin <nbin; ibin++){
        ...
        for(int ir = 0 ; ir <nrbin; ir++){
            ...
            delete SyDiffJS[ibin][ir];
            delete AsyDiffJS[ibin][ir];
            delete bkgSyDiffJS[ibin][ir];
            delete bkgAsyDiffJS[ibin][ir];
...
void hist_class::Write()
{
  ...
  //  TFile *out_file = new TFile(Form("/Users/ymao/group/CMS/anaOutputs/%s",out_name.Data()), "RECREATE");
        TFile *out_file = new TFile(Form("/net/hidsk0001/d00/scratch/maoyx/JetShape/hiForest2/%s",out_name.Data()), "RECREATE");
    for(int ibin = 0 ; ibin <nbin; ibin++){
        NEvents[ibin]->Write();
        NJets[ibin]->Write();
        Ntrack[ibin]->Write();
        ...
        for(int ir = 0 ; ir <nrbin; ir++){
            ...
            SyDiffJS[ibin][ir]->Write();
            AsyDiffJS[ibin][ir]->Write();
            bkgSyDiffJS[ibin][ir]->Write();
            bkgAsyDiffJS[ibin][ir]->Write();
...
int main()
{
   ...
   //   dataPath="/mnt/hadoop/cms/store/user/frankmalocal/forest" ; //hiForest2 files
    else {
    ...
    if(my_hists->IsMC!=kTRUE){  //real data analysis
        ...
        //    intputFile="HiForest-promptskim-hihighpt-hltjet80-pt90-v2_v3_part2.root" ;
        else 
            intputFile="merged_pp2760_AllPhysics_Part_Prod03.root";
        // intputFile="mit_pelin_dijet_hiforest_pp_276TeV_data.root";
        // intputFile="hiforest_pp_276TeV_data.root";
    }
    else { //MC sample
        if(coll=="HI")             
          //  intputFile=Form("HiForest2_DijetPt%d_Hydjet_Drum_test.root", pthat);
          intputFile="merged_HydjetDrum.root";
        else 
            //     intputFile="merged_hiForest.root";
           //   intputFile="Pythia80_signal01_HiForest2_v19_0.root";
...
//    TTree *tr_tree = (TTree*) my_file->Get("mergedTrack/trackTree");
//    MergedTrack * my_tr = new MergedTrack(tr_tree);
...
//            int jetTr2 = trigSel->HLT_Jet60_v1 ;
     ...
     //   if(my_hists->IsMC==kTRUE && DoGenAna)my_GenPart->fChain->GetEntry(evi);
        ...
        for(int ijet = 0 ; ijet <my_ct->nref; ijet++){
            ...
            //remove flucluation with too large pt from small MC pthat sample
            if(my_hists->IsMC==kTRUE && my_ct->jtpt[ijet]>5*pthat) continue ;
...
//            if(jtrdphi<=-TMath::Pi()) jtrdphi+=TMath::TwoPi();            
        ...
        for(int ir = 0 ; ir <nrbin; ir++){
            ...
            if(Aj<0.3){
                my_hists->SyDiffJS[curr_bin][ir]->Fill(lead_pt, rho[ir],weight);
                my_hists->bkgSyDiffJS[curr_bin][ir]->Fill(lead_pt, bkgrho[ir],weight);
            }
            else {
                my_hists->AsyDiffJS[curr_bin][ir]->Fill(lead_pt, rho[ir],weight);
                my_hists->bkgAsyDiffJS[curr_bin][ir]->Fill(lead_pt, bkgrho[ir],weight);
            }
            if(dEtaBin!=-1){
...
//            if(TMath::Abs(gensubjet_eta) < 0.3) continue ;                
            ...
            for(int ipart = 0 ; ipart < my_tr->nParticle ; ipart++){
...
//                if(my_tr->highPurity[ipart]==0) continue ;
//                if(my_tr->trkAlgo[ipart]!=4.0) continue ;
                if(my_tr->pNRec[ipart]<=0.0) continue ;
...
//                        double genbkgsub_dr = TMath::Sqrt((gen_phi-bkg_subphi)*(gen_phi-bkg_subphi)+(gen_eta-bkg_subeta)*(gen_eta-bkg_subeta)); 
            ...
            my_hists->genSumPt[curr_bin]->Fill(sumpt,gensumpt, weight);
