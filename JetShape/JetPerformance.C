...
#include "class_def/MergedTrack.h"
#include "class_def/icPu5Jet.h"
...
hist_class::hist_class(TString the_desc)
{
    ...
    for(int ibin = 0 ; ibin <nbin; ibin++){
        ...
        rawpt[ibin] = new TH1F(Form("rawjetpt_%d-%d%%",centr[ibin],centr[ibin+1]), Form("rawjetpt_%d-%d%%",centr[ibin],centr[ibin+1]), 500, 0., 500.);
        rawpt[ibin]->Sumw2();
...
void hist_class::Write()
{
    ...
    if(IsMC){
        if(etalimit==etacut)
            out_name=Form("%s%s_PFJetPt%.f_%.fSimTrk%.fEta%.f_RCbkgEtaBound%.f_CenBin%d_Nrbin%d_Pthat%d%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin,pthat, intputFile.Data());
        else 
            out_name=Form("%s%s_PFJetPt%.f_%.fSimTrk%.fEta%.f_ERbkgEtaBound%.f_CenBin%d_Nrbin%d_Pthat%d%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin,pthat, intputFile.Data());
    }
    else {
        if(etalimit==etacut)
            out_name=Form("%s%s_PFJetPt%.f_%.fTrk%.fEta%.f_RCbkgEtaBound%.f_CenBin%d_Nrbin%d_%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin, intputFile.Data());
        else 
            out_name=Form("%s%s_PFJetPt%.f_%.fTrk%.fEta%.f_ERbkgEtaBound%.f_CenBin%d_Nrbin%d_%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin, intputFile.Data());   
    }
...
int main()
{
    ...
    //  dataPath="/mnt/hadoop/cms/store/user/velicanu/forest" ;//mit PbPb data path
     ...
     //   dataPath= "/Users/ymao/group/CMS/hiForest"; //local analysis
               if(coll=="HI") 
               dataPath= Form("/mnt/hadoop/cms/store/user/yenjie/Hydjet1.8/Z2/Dijet80/tracking_v1/hiForest20_merged"); //mit test file path for HI  
        ...
        //        //        dataPath= Form("/mnt/hadoop/cms/store/user/mnguyen/MC_Production/pythia%d",pthat); //mit path for pp
               //    dataPath= Form("/net/hisrv0001/home/icali/hadoop/Pythia/Z2/ppDijet_merged"); //mit path for pp
                   dataPath= Form("/net/hisrv0001/home/yetkin/hidsk0001/prod/DrumMix/Z2/pythia%d_signal01/forest19", pthat); //mit path for pp test file
    }
    if(my_hists->IsMC!=kTRUE){  //real data analysis
        ...
        //   intputFile="HiForestDijet_v7.root";
           intputFile="promptskim-hihighpt-hltjet80-pt90-v1_v2_part.root";
        else 
        ...
        if(coll=="HI")             
         ...
         //  intputFile="merged_HydjetDrum.root";
          intputFile="HiForest2_DijetPt80_Hydjet_Drum_test.root";
        else 
         ...
         //  intputFile=Form("mit_ivan_pp276Dijet%d_merged.root",pthat);
            intputFile=Form("Pythia%d_signal01_HiForest2_v19.root",pthat);  
    }
...
//    TTree *inp_tree = (TTree*) my_file->Get("icPu5JetAnalyzer/t");
//    icPu5Jet * my_ct = new icPu5Jet(inp_tree);    
      ...
      //   std::cout <<"!!!!!!!!!!!!!!still working\n";
        ...
        for(int j4i = 0; j4i < my_ct->nref ; j4i++) {
            ...
            //remove flucluation with too large pt from small MC pthat sample
              ...
              my_hists->rawpt[curr_bin]->Fill(my_ct->rawpt[j4i], weight); 
           if(jet_pt >lead_pt){
                lead_pt=jet_pt;
                lead_index=j4i;
            }
...
//            if(jtrdphi<=-TMath::Pi()) jtrdphi+=TMath::TwoPi();            
          ...
          if(my_hists->IsMC==kTRUE&&DoGenAna){ 
            ...
            for(int ir = 0 ; ir <nrbin ; ir++){
                ...
                double gen_pt = my_tr->pPt[itr];
                double gen_phi = my_tr->pPhi[itr];
                double gen_eta = my_tr->pEta[itr];
                ...
                if(gendr<=conesize && gendr2<=conesize) {
                    cout <<"!!!! two jets are overlap with gen particle: "<<itr <<"eta: " <<gen_eta <<" phi:" <<gen_phi << endl;
                    continue ;
                }       
                if(gendr <=conesize){
                    gensumpt+=gen_pt ;
