...
void hist_class::Write()
{
    ...
    if(IsMC){
        if(etalimit==etacut)
            out_name=Form("%s%s_PFJetPt%.f_%.fTrk%.fEta%.f_RCbkgEtaBound%.f_CenBin%d_Nrbin%d_Pthat%d%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin,pthat, intputFile.Data());
        else 
            out_name=Form("%s%s_PFJetPt%.f_%.fTrk%.fEta%.f_ERbkgEtaBound%.f_CenBin%d_Nrbin%d_Pthat%d%s",dataType.Data(),coll.Data(),leadingjetcut,subleadjetcut,trackcut, etacut,etalimit*10, nbin,nrbin,pthat, intputFile.Data());
    }
    else {
        if(etalimit==etacut)
