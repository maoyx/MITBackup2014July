...
class TrkCorrHisAna
{
  public:
    ...
    // methods
    TrkCorrHisAna(TString name, TFile * outf, float jetPtMin=40, TString coll="PbPb");
...
TrkCorrHisAna::TrkCorrHisAna(TString name, TFile * outf, float jPtMin, TString coll) :
  name_(name),
  coll_(coll),
  trkPhiMode_(false),
  jetPtMin(jPtMin)
{
 ...
 //  const Int_t numHigtPtBins=18;
   Double_t highPtBins[] = {2.0, 2.5, 3.0, 4., 5., 6., 8., 10.,12.,15.,20.,25.,30.,45.,60.,80.,100., 120.,150., 180.,300.,500.};
  const Int_t numHigtPtBins=sizeof(highPtBins)/sizeof(Double_t)-1 ;
...
//   jetBins.insert(jetBins.end(),jBins,jBins+numJetBins+1);
   const Int_t numJetBins=10;
   Float_t jBins[numJetBins+1] = {0,40, 60, 80,100, 120, 160, 200, 300, 500, 1000};
   jetBins.insert(jetBins.end(),jBins,jBins+numJetBins+1);
...
void TrkCorrHisAna::DeclareHistograms()
{
   ...
   cout << "===== " << name_ << " =====" << endl;
	...
	for (Int_t i=0; i<(Int_t)ptBins.size(); ++i) cout << ptBins[i] << " ";
	cout << endl << "Eta " << etaBins.size()-1 << " bins:";
	for (Int_t i=0; i<(Int_t)etaBins.size(); ++i) cout << etaBins[i] << " ";
	cout << endl << "zaxis: " << zBins.size()-1 << " bins:";
	for (Int_t i=0; i<(Int_t)zBins.size(); ++i) cout << zBins[i] << " ";
	cout << endl;
