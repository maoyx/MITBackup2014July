...
{   
 ...
 //  if (track.trkAlgo[j]>=4&&!(track.highPurity[j])) return 0;
   if (!(track.highPurity[j])) return 0;
    if(track.trkPtError[j]/track.trkPt[j]>=0.1 || TMath::Abs(track.trkDz1[j]/track.trkDzError1[j])>=3.0 || TMath::Abs(track.trkDxy1[j]/track.trkDxyError1[j])>=3.0) return 0 ; //ridge cut for tracks              
 
   if (initialized) {
      // check if the leading and subleading jet is found
      if (leadingJetPtForTrkCor<-1) {
      // Select leading and subleading jet
         for (int k=0; k<ak3PF.nref; k++) {
            if (fabs(ak3PF.jteta[k])>2) continue;
            if (ak3PF.jtpt[k]>leadingJetPtForTrkCor) {
     	       leadingJetPtForTrkCor = ak3PF.jtpt[k];
	       leadingJetEtaForTrkCor = ak3PF.jteta[k];
	       leadingJetPhiForTrkCor = ak3PF.jtphi[k];
   	    }   
	    if (ak3PF.jtpt[k]>subleadingJetPtForTrkCor && ak3PF.jtpt[k] < leadingJetPtForTrkCor) {
	       subleadingJetPtForTrkCor = ak3PF.jtpt[k];
	       subleadingJetEtaForTrkCor = ak3PF.jteta[k];
	       subleadingJetPhiForTrkCor = ak3PF.jtphi[k];
            }
	    if (ak3PF.jtpt[k]<subleadingJetPtForTrkCor) break;	 
         }
      }
   } else {
      cout <<"FATAL: Tracking correction is not initialized. Return 0, Please do HiForest->InitTree()."<<endl;
   ...
   //   trkWt = trackCorrections[leadingSet]->GetCorr(track.trkPt[j],track.trkEta[j],0,evt.hiBin);
    ...
    //  trkWt = trackCorrections[subleadingSet]->GetCorr(track.trkPt[j],track.trkEta[j],0,evt.hiBin);
   } else {
